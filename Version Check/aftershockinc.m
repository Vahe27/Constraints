function y = aftershockinc(Y,w,ee,preps,n,Capflag);
...
% Last Update 16.10.2017 the exemption now is not xi but xi*w0
...
for ii = 1:n
    ...
    y1(:,:,ii) = max(piown - ee(ii),min(xi*w,yown));
    y2(:,:,ii) = max(piemp - ee(ii),min(xi*w,yemp));
