...
OCC = FCH(BIGZ,BIGA,bigprob,bigkap,TOTOCC(:,tt));

indOCC = find(floor(OCC)~=OCC);
indLOC = TOTOCC(indOCC,tt);


NT  = FN(BIGZ(indOCC),BIGA(indOCC),bigprob(indOCC),bigkap(indOCC));
ST  = FS(BIGZ(indOCC),BIGA(indOCC),bigprob(indOCC),bigkap(indOCC));
BNT = FBN(BIGZ(indOCC),BIGA(indOCC),bigprob(indOCC),bigkap(indOCC));
WT  = FW(BIGZ(indOCC),BIGA(indOCC),bigprob(indOCC),bigkap(indOCC));
BBT = FBB(BIGZ(indOCC),BIGA(indOCC),bigprob(indOCC),bigkap(indOCC)); 

TEMP = OCC(indOCC);
DUMM  = -100*ones(length(TEMP),1);


[VALS TEMP] = max([NT DUMM ST BNT],[],2);

TEMP2 = TEMP(indLOC==4);

TEMP2(BBT(indLOC==4)>VALS(indLOC==4)) = 4;
TEMP(indLOC==4)=TEMP2;
TEMP2 = TEMP(indLOC==6| indLOC==8);
TEMP2(WT(indLOC==6| indLOC==8)>VALS(indLOC==6| indLOC==8))=2;
TEMP(indLOC==6| indLOC==8)=TEMP2;
TEMP2 = TEMP(indLOC==8);
TEMP2(BBT(indLOC==8)>VALS(indLOC==8)) = 4;
TEMP(indLOC==8)=TEMP2;
OCC(indOCC) = TEMP;
