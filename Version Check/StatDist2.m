...
OCCINDS = zeros(nz,na,nprob,nkap,4);
OCCINDS(:,:,:,:,1) = OCCNEMNB;
OCCINDS(:,:,:,:,2) = OCCNEMB;
OCCINDS(:,:,:,:,3) = OCCEMPNB;
OCCINDS(:,:,:,:,4) = OCCEMPB;
DUMBOX = repmat(reshape([2 3 6 8],1,1,1,1,4),nz,na,nprob,nkap,1);


bige  = [bigeprobs zeros(NN,1)];
biga  = [biga zeros(NN,TT)];
POSB  = POSBSTAT;

FCH   = griddedInterpolant(repmat(z_array,1,1,1,1,4),repmat(a_array,1,1,1,1,4),...
repmat(prob_array,1,1,1,1,4),repmat(kap_array,1,1,1,1,4),DUMBOX,OCCINDS);
