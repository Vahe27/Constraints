...
lambda = 0.15; % avg emp duration 2.5y Shimer (2005)
mu     = 0.3; % calculations in the notes
...
ZDist    = 2; % Pareto, if 1. Quadrature if 2
etta     = 13;
sigz     = 1/varphi;
...
mueps    = 1;
sigeps   = 30;
zetta    = [0.7 0.3]; % The likelihood that conditional on shock realizing, it will be low
epsilon  = [0 3 10]; 

EPSdist  = 3; % 3 if pareto distributed
KAPdist  = 2;
...
X        = prodshock(mueps,sigeps,nprob,EPSdist,upperp); % P and epsilon can also be vectors with size neps
P        = X';
clear X

X        = zdist(sigz,etta,nz,ZDist,upperz);
Pz       = X(:,1);
zgrid    = X(:,2);
clear X
...
Zint  = zint(bigz(:,end)); %Zint(1) = (z*varphi)^1/(1-gama)
Zint1 = Zint(:,1);
Zint2 = Zint(:,2);
Zm1   = Zint(:,3);
Zm2   = Zint(:,4);
...
SEMP     = profitcalc3(zgrid*varphi,agrid,kgrid(1,:),r0(1,:),w0);

BUS      = profitcalc3(zgrid,agrid,kgrid(2,:),r0(2,:),w0);

BUSN     = profitcalc3_entry(zgrid,agrid,kgrid(2,:),r0(2,:),w0); 
...
EVwN = (1-PSI) * VwN + PSI * repmat(reshape(Pz'*reshape(VwN,nz,na...
    *nkap*nprob),1,na,nprob,nkap),nz,1,1,1);
EVuN = (1-PSI) * VuN + PSI * repmat(reshape(Pz'*reshape(VuN,nz,na...
    *nkap*nprob),1,na,nprob,nkap),nz,1,1,1);
EVwB = (1-PSI) * VwB + PSI * repmat(reshape(Pz'*reshape(VwB,nz,na...
    *nkap*nprob),1,na,nprob,nkap),nz,1,1,1);
EVuB = (1-PSI) * VuB + PSI * repmat(reshape(Pz'*reshape(VuB,nz,na...
    *nkap*nprob),1,na,nprob,nkap),nz,1,1,1);
...
clear iminW imaxW iminS imaxS iminB imaxB ilowW iupW ilowS iupS ilowB...
    iupB Smin Smax Wmin Wmax Bmin Bmax Uuk Uwk UbNBk
...
Fm1    = griddedInterpolant(Iint(:,1),Iint(:,7));
Fm2    = griddedInterpolant(Iint(:,2),Iint(:,8));

BZ3 = BIGZ(OCC==3)*varphi;
BA3 = BIGA(OCC==3);
ZI1 = Zint1(OCC==3);
BZ4 = BIGZ(OCC==4);
BA4 = BIGA(OCC==4);
...
X  = KLMCMCINT3(BZ3,BA3,kgrid(1,:),r0(1,:),w0...
    ,Falfa1,Fgama1,ZI1,Fm1,Zm1);
LDS       = X(:,1);
...
X  = KLMCMCINT3(BZ4,BA4,kgrid(2,:),r0(2,:),w0...
    ,Falfa2,Fgama2,ZI2,Fm2,Zm2);
...
avgr =avgrates(r0,w0,KBOR,kgrid);
...
disp(['.... unemp' '......emp' '....subsemp' '.....semp' '.....wage'...
    '...borrowingR' '...tenure'])
disp([ unemp LS sharesubsemp semp w0 avgr(1) avgworktenure]) 
...
clear a_array z_array kap_array prob_array zarrayintintS probarrayintS...
    ...
    busincNP EABB EABN EBBold EBNold IB IBnew IhireB ...
    IhireNB IhireS MAXESN OCCEMPB OCCEMPNB OCCNEMB OCCNEMNB probmtx ...
    TTotS UbNBk VALMAT VuB VuBold VuN VuNold VwB VwBold VwN VwNold...
    ...
    BZ3 BZ4 fm1 fm2 kaploc ploc POSBSTAT tenext zloc Zint1 Zin2 Zm1 Zm2 ZM2...
    ZS
