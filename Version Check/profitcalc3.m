function y = profitcalc3(z,a,k,r,w);
...
% update 1/12 I add the option to both hire labor and supply part of labor
in the production process, so that m>0 and l>0, this can happen if the
individual is constrained, so the if he is not, I don't consider the case.
If he is constrained, I calculate the optimal m and lm and the profit, and
calculate borrowed amount base on that
%}
...
kownacc = repmat((alfa/(r_bar + deltta))^(1/(1-alfa)) * Gama^(1/(1-alfa))...
    * z .^ (1/(1-alfa)),1,na);
...
%%%%
tempm = w^(1/gama)/gama * borrowerz.^(-1/gama).*invest.^(-alfa/gama);

m = max(0,1-tempm);
lm = max(0,tempm*(gama+1)-1);

pimatrixm = (1-m).*borrowerz.*invest.^alfa.*lm.^gama - paymentk - lm*w +...
    (1-deltta).*invest;

pimatrixemp = max(pimatrixm,pimatrixemp);
...
% borrowed = Ihire.*ystarE  + (1-Ihire).*ystarO; 
