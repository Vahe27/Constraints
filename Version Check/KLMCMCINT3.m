function y = KLMCMCINT3(z,a,k,r,w,Falfa,Fgama,Zgama,Fam,Zm)
...
pimatrixemp = zgama.*invgama*...
    ...
    *invest - paymentk;
%%%%
zm = repmat(Zm(IndBemp),1,nk);
invm = reshape(Fam(invest(:)),nborrow,nk);

tempm = w^(1/gama)/gama * zm.*invm;

m = max(0,1-tempm);
lm = max(0,tempm*(gama+1)-1);

pimatrixm = w*tempm - paymentk - lm*w + (1-deltta).*invest;
...
LM = lm([1:nborrow]'+(Ikemp-1)*nborrow);
Lownind = pimatrixm([1:nborrow]'+(Ikemp-1)*nborrow)>...
    pimatrixemp([1:nborrow]'+(Ikemp-1)*nborrow);

LTEMP = (gama/w * z(IndBemp)).^(1/(1-gama)) .*...
    (invest([1:nborrow]'+(Ikemp - 1)*nborrow).^(alfa/(1-gama)));

LabD(IndBemp,1) = Lownind.*LM + (1-Lownind).*LTEMP;
...
LabD = LabD.*occindex + Lown.*(1-occindex);
